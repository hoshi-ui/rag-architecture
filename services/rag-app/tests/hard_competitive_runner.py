...
CASES_PATH = os.path.join(THIS_DIR, "hard_competitive_cases.json")
REPORT_PATH = os.path.abspath(os.path.join(THIS_DIR, "..", "uploads", "hard_competitive_report.json"))
...
def run_retrieve(query: str, top_k: int = 10, enable_rerank: bool = True) -> Dict[str, Any]:
    ...
    response = requests.post(
        f"{BASE_URL}/retrieve",
        json={"query": query, "user_id": "hard_competitive_eval", "top_k": top_k, "enable_rerank": enable_rerank},
        timeout=180,
    )
